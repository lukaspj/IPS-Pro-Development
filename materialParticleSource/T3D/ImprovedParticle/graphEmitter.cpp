...
ConsoleDocClass( GraphEmitter,
	"@brief This object spawns particles based on mathematical expressions.\n\n"
...
ConsoleDocClass( GraphEmitterData,
	"@brief Defines particle emission properties such as ejection period "
...
void GraphEmitterData::packData(BitStream* stream)
{
	...
	stream->writeInt(ejectionPeriodMS * 100, 10);
	stream->writeInt(periodVarianceMS * 100, 10);
	...
	stream->writeInt( softnessDistance * 1000, 15 );
	stream->writeInt( ambientFactor * 1000, 15 );

	stream->writeFlag(overrideAdvance);
	stream->writeFlag(orientParticles);
	stream->writeFlag(orientOnVelocity);
	stream->writeInt( lifetimeMS, 10 );
	stream->writeInt( lifetimeVarianceMS, 10 );
	stream->writeFlag(useEmitterSizes);
	stream->writeFlag(useEmitterColors);

	stream->writeInt(dataBlockIds.size(), 15);
	for (U32 i = 0; i < dataBlockIds.size(); i++)
		stream->writeInt(dataBlockIds[i], 15);
...
void GraphEmitterData::unpackData(BitStream* stream)
{
	...
	ejectionPeriodMS = stream->readInt(10) / 100.0f;
	periodVarianceMS = stream->readInt(10) / 100.0f;
	...
	softnessDistance = stream->readInt( 15 ) / 1000.0f;
	ambientFactor = stream->readInt( 15 ) / 1000.0f;

	overrideAdvance = stream->readFlag();
	orientParticles = stream->readFlag();
	orientOnVelocity = stream->readFlag();
	lifetimeMS = stream->readInt( 10 );
	lifetimeVarianceMS = stream->readInt( 10 );
	useEmitterSizes = stream->readFlag();
	useEmitterColors = stream->readFlag();

	U32 size = stream->readInt(15);
	dataBlockIds.setSize(size);
	for (U32 i = 0; i < dataBlockIds.size(); i++)
		dataBlockIds[i] = stream->readInt( 15 );
	...
	GraphEmitterNode* nodeDat)
{
	...
	if(!nodeDat->currentlyShuttingDown() && nodeDat)
	{
