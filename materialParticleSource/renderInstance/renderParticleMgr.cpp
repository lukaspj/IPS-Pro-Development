...
void RenderParticleMgr::renderInstance(ParticleRenderInst *ri, SceneRenderState *state)
{
   ...
   if(ri->systemState != PSS_AwaitingCompositeDraw)
   {
	  ...
	  MatrixSet &matrixSet = getRenderPass()->getMatrixSet();
		...
		matrixSet.setWorld(*ri->objectToWorld);
		matrixSet.setView(*ri->worldToCamera);
		matrixSet.setProjection(*ri->projection);
		ri->matInst->setTransforms(matrixSet, state);
	  ri->matInst->setupPass(state, sgData );
	  ri->matInst->setSceneInfo(state, sgData);
	  ri->matInst->setTextureStages( state, sgData );
	  ri->matInst->setBuffers( ri->vertBuff, ri->primBuff );
