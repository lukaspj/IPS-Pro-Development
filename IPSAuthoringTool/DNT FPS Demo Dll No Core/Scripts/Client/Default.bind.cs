...
namespace DNT_FPS_Demo_Game_Dll.Scripts.Client
{
    public partial class Main : TorqueScriptTemplate
    {
        ...
        {
            console.Call("MessageBoxYesNo",
                     console.GetVarString("$Server::ServerType") == "SinglePlayer"
                         ? new[] { "Exit", "Exit from this Mission?", "disconnect();", "" }
                         : new[] { "Disconnect", "Disconnect from the server?", "disconnect();", "" });
            return string.Empty;
        }
        ...
        public void toggleZoomFOV()
        {
            ...
            if (console.GetVarBool("ServerConnection.zoomed"))
                ...
                console.Call("setFov", new string[] { cfov.AsString() });
            else
                console.Call("setFov", new string[] { console.GetVarString("ServerConnection.getControlCameraDefaultFov()") });
        }
        ...
        public void turnOffZoom()
        {
            ...
            console.Call("setFov", new string[] { GameConnection.getControlCameraDefaultFov("ServerConnection").AsString() });
        ...
        public void toggleZoom(string val)
        {
            if (val.AsBool())
            {
                ...
                console.Call("setFov", new string[] { console.GetVarString("$Player::CurrentFOV") });
        ...
        public void cycleDebugRenderMode(string val)
        {
            ...
            if (!console.isObject("ChatHud")) return;
        ...
        public void carjack()
        {
            ...
            if (console.GetClassName(player) != "Player") return;
        ...
        [Torque_Decorations.TorqueCallBack("", "", "toggleCursor", "(val)", 1, 11001, false)]
        public void toggleCursor(string val)
        {
            if (val == "1")
                hideCursor();
            else
        ...
        [Torque_Decorations.TorqueCallBack("", "", "resetEmitter", "(val)", 1, 11001, false)]
        public void resetEmitter(string val)
        {
            if (val == "1")
                console.Eval("theEmitter.reset();");
        }

        [Torque_Decorations.TorqueCallBack("", "", "mouseWheelScroll", "(val)", 1, 11001, false)]
        public void mouseWheelScroll(string val)
        {
            float CurrentFOV = console.GetVarFloat("$cameraFov");
            float rollAdj = getMouseAdjustAmount(val).AsFloat();
            console.print("Val: " + val.AsString() + " rollAdj: " + rollAdj.AsString());
            CurrentFOV -= rollAdj*2;
            if (CurrentFOV < 0.5)
                CurrentFOV = 10;
            if (CurrentFOV > 100)
                CurrentFOV = 100;
            console.Call("setFov", new string[] { CurrentFOV.AsString() });
        }
        ...
        public void init_Default_Bind()
        {
            ...
            ActionMap.bind("moveMap", "mouse", "zaxis", "mouseWheelScroll");
            ActionMap.bind("GlobalActionMap", "mouse", "button0", "toggleCursor");
            ActionMap.bind("moveMap", "keyboard", "r", "resetEmitter");
