...
IMPLEMENT_CO_DATABLOCK_V1(EffectParticleData);

ConsoleDocClass( EffectParticleData,
...
// Constructor
//-----------------------------------------------------------------------------
EffectParticleData::EffectParticleData()
{
   mEffectData = NULL;
}
...
// initPersistFields
//-----------------------------------------------------------------------------
void EffectParticleData::initPersistFields()
{
   addProtectedField( "times", TYPEID< F32 >(), Offset(times, EffectParticleData), &protectedSetTimes, 
   ...
   addField( "EffectDatablock", TYPEID< ParticleEffectData >(), Offset(mEffectData, EffectParticleData), "");
...
// Pack data
//-----------------------------------------------------------------------------
void EffectParticleData::packData(BitStream* stream)
{
   ...
   if(stream->writeFlag(mEffectData))
      stream->writeRangedU32(mEffectData->getId(), DataBlockObjectIdFirst,
...
// Unpack data
//-----------------------------------------------------------------------------
void EffectParticleData::unpackData(BitStream* stream)
{
...
bool EffectParticleData::protectedSetTimes( void *object, const char *index, const char *data) 
{
   EffectParticleData *pData = static_cast<EffectParticleData*>( object );
   F32 val = dAtof(data);
   U32 i;
...
// preload
//-----------------------------------------------------------------------------
bool EffectParticleData::preload(bool server, String &errorStr)
{
...
bool EffectParticleData::reload(char errorBuffer[256])
{
   bool error = Parent::reload(errorBuffer);
   return !error;
}

void EffectParticleData::preloadVector(Vector<ParticleData*> _particleDataBlocks, U32 mask, ParticleEmitterData* emitterData)
{
}

DefineEngineMethod(EffectParticleData, reload, void, (),,
