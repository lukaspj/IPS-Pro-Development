...
IMPLEMENT_CO_DATABLOCK_V1(TSShapeParticleData);

ConsoleDocClass( TSShapeParticleData,
...
// Constructor
//-----------------------------------------------------------------------------
TSShapeParticleData::TSShapeParticleData()
{
   mShape = NULL;
}
...
// initPersistFields
//-----------------------------------------------------------------------------
void TSShapeParticleData::initPersistFields()
{
   addProtectedField( "times", TYPEID< F32 >(), Offset(times, TSShapeParticleData), &protectedSetTimes, 
   ...
   addField( "ShapeFile", TypeStringFilename, Offset(mShapeFile, TSShapeParticleData), "" );
...
// Pack data
//-----------------------------------------------------------------------------
void TSShapeParticleData::packData(BitStream* stream)
{
...
// Unpack data
//-----------------------------------------------------------------------------
void TSShapeParticleData::unpackData(BitStream* stream)
{
...
void TSShapeParticleData::createShape()
{
   // If this is the same shape then no reason to update it
   if ( mShapeFile.equal( mShape.getPath().getFullPath(), String::NoCase ) )
      return;
   mShape = NULL;
...
bool TSShapeParticleData::protectedSetTimes( void *object, const char *index, const char *data) 
{
   TSShapeParticleData *pData = static_cast<TSShapeParticleData*>( object );
   F32 val = dAtof(data);
   U32 i;
...
// onAdd
//-----------------------------------------------------------------------------
bool TSShapeParticleData::onAdd()
{
   if (!Parent::onAdd())
      return false;
...
// preload
//-----------------------------------------------------------------------------
bool TSShapeParticleData::preload(bool server, String &errorStr)
{
...
bool TSShapeParticleData::reload(char errorBuffer[256])
{
   bool error = Parent::reload(errorBuffer);
   return !error;
}

void TSShapeParticleData::preloadVector(Vector<ParticleData*> _particleDataBlocks, U32 mask, ParticleEmitterData* emitterData)
{
}

DefineEngineMethod(TSShapeParticleData, reload, void, (),,
