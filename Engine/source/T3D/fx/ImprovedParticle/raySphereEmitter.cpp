...
IMPLEMENT_CO_DATABLOCK_V1(RaySphereEmitterData);
IMPLEMENT_CONOBJECT(RaySphereEmitter);

ConsoleDocClass( RaySphereEmitter,
...
ConsoleDocClass( RaySphereEmitterData,
...
// SphereEmitterData
//-----------------------------------------------------------------------------
RaySphereEmitterData::RaySphereEmitterData()
{
...
// initPersistFields
//-----------------------------------------------------------------------------
void RaySphereEmitterData::initPersistFields()
{
	addGroup( "RaySphereEmitterData" );

	addField( "thetaMin", TYPEID< F32 >(), Offset(thetaMin, RaySphereEmitterData),
		"Minimum angle, from the horizontal plane, to eject from." );

	addField( "thetaMax", TYPEID< F32 >(), Offset(thetaMax, RaySphereEmitterData),
		"Maximum angle, from the horizontal plane, to eject particles from." );

	addField( "phiReferenceVel", TYPEID< F32 >(), Offset(phiReferenceVel, RaySphereEmitterData),
		"Reference angle, from the vertical plane, to eject particles from." );

	addField( "phiVariance", TYPEID< F32 >(), Offset(phiVariance, RaySphereEmitterData),
		"Variance from the reference angle, from 0 - 360." );

	endGroup( "RaySphereEmitterData" );
...
// packData
//-----------------------------------------------------------------------------
void RaySphereEmitterData::packData(BitStream* stream)
{
...
// unpackData
//-----------------------------------------------------------------------------
void RaySphereEmitterData::unpackData(BitStream* stream)
{
...
// onAdd
//-----------------------------------------------------------------------------
bool RaySphereEmitterData::onAdd()
{
	if( thetaMin < 0.0f )
	{
...
// RaySphereEmitter
//-----------------------------------------------------------------------------
RaySphereEmitter::RaySphereEmitter()
{
}
...
// addParticle
//-----------------------------------------------------------------------------
bool RaySphereEmitter::addParticle(const Point3F& pos,
	const Point3F& axis,
	const Point3F& vel,
	const Point3F& axisx, 
	const MatrixF &trans)
{
	RaySphereEmitterData* DataBlock = getDataBlock();
   ...
   Point3F Start = pos;
   Point3F EndPos = pos + (ejectionAxis * DataBlock->ejectionOffset);

   RayInfo rInfo;
   bool hit = gClientContainer.castRay(Start, EndPos, SceneObjectTypes::ShapeBaseObjectType, &rInfo);
   if(!hit)
      return false;
	...
	ParticleEmitterNode* pnodeDat)
{
   ...
   Point3F EndPos = pos + (ejectionAxis * DataBlock->ejectionOffset);
	...
	if(standAloneEmitter)
      EndPos = pos + (ejectionAxis * sa_ejectionOffset);
	else
      EndPos = pos + (ejectionAxis * DataBlock->ejectionOffset);

   RayInfo rInfo;
   bool hit = gClientContainer.castRay(Start, EndPos, ShapeBaseObjectType | PlayerObjectType, &rInfo);
   if(!hit)
      return false;
   Particle* pNew = mParticleManager->AddParticle();
...
ParticleEmitter* RaySphereEmitterData::createEmitter() { return new RaySphereEmitter; }

DefineEngineMethod(RaySphereEmitterData, reload, void,(),,
