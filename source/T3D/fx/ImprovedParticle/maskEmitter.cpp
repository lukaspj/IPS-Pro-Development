...
Point3F MaskEmitter::mWindVelocity( 0.0, 0.0, 0.0 );
const F32 MaskEmitter::AgedSpinToRadians = (1.0f/1000.0f) * (1.0f/360.0f) * M_PI_F * 2.0f;
...
ConsoleDocClass( MaskEmitter,
	...
	"particle emitting object in the scene, use a MaskEmitterNode instead.\n\n"

	"This class is the main interface for creating particles - though it is "
	"usually only accessed from within another object like MaskEmitterNode "
...
ConsoleDocClass( MaskEmitterData,
	...
	"and velocity for a MaskEmitter.\n\n"

	"@tsexample\n"
	"datablock MaskEmitterData( GrenadeExpDustEmitter )\n"
	"{\n"
	"   ejectionPeriodMS = 1;\n"
	"   periodVarianceMS = 0;\n"
	"   ejectionVelocity = 15;\n"
...
typedef ParticleRenderInst::BlendStyle maskParticleBlendStyle;
DefineEnumType( maskParticleBlendStyle );
...
void MaskEmitterData::initPersistFields()
{
	...
	addGroup( "MaskEmitterData" );

	addField("pixelMaskPath", TYPEID< StringTableEntry >(), Offset(pixelMaskPath,   MaskEmitterData),
		"Time (in milliseconds) between each particle ejection." );

	addField("ejectionPeriodMS", TYPEID< S32 >(), Offset(ejectionPeriodMS,   MaskEmitterData),
		"Time (in milliseconds) between each particle ejection." );

	addField("periodVarianceMS", TYPEID< S32 >(), Offset(periodVarianceMS,   MaskEmitterData),
		"Variance in ejection period, from 1 - ejectionPeriodMS." );

	addField( "ejectionVelocity", TYPEID< F32 >(), Offset(ejectionVelocity, MaskEmitterData),
		"Particle ejection velocity." );

	addField( "velocityVariance", TYPEID< F32 >(), Offset(velocityVariance, MaskEmitterData),
		"Variance for ejection velocity, from 0 - ejectionVelocity." );

	addField( "ejectionOffset", TYPEID< F32 >(), Offset(ejectionOffset, MaskEmitterData),
		"Distance along ejection Z axis from which to eject particles." );

	addField( "radius", TYPEID< F32 >(), Offset(radius, MaskEmitterData),
		"Distance along ejection Z axis from which to eject particles." );

	addField( "softnessDistance", TYPEID< F32 >(), Offset(softnessDistance, MaskEmitterData),
	...
	addField( "ambientFactor", TYPEID< F32 >(), Offset(ambientFactor, MaskEmitterData),
	...
	addField( "overrideAdvance", TYPEID< bool >(), Offset(overrideAdvance, MaskEmitterData),
	...
	addField( "orientParticles", TYPEID< bool >(), Offset(orientParticles, MaskEmitterData),
		"If true, Particles will always face the camera." );

	addField( "orientOnVelocity", TYPEID< bool >(), Offset(orientOnVelocity, MaskEmitterData),
		"If true, particles will be oriented to face in the direction they are moving." );

	addField( "particles", TYPEID< StringTableEntry >(), Offset(particleString, MaskEmitterData),
	...
	addField( "lifetimeMS", TYPEID< S32 >(), Offset(lifetimeMS, MaskEmitterData),
		"Lifetime of emitted particles (in milliseconds)." );

	addField("lifetimeVarianceMS", TYPEID< S32 >(), Offset(lifetimeVarianceMS, MaskEmitterData),
		"Variance in particle lifetime from 0 - lifetimeMS." );

	addField( "useEmitterSizes", TYPEID< bool >(), Offset(useEmitterSizes, MaskEmitterData),
	...
	addField( "useEmitterColors", TYPEID< bool >(), Offset(useEmitterColors, MaskEmitterData),
	...
	addField( "blendStyle", TYPEID< ParticleRenderInst::BlendStyle >(), Offset(blendStyle, MaskEmitterData),
		"String value that controls how emitted particles blend with the scene." );

	addField( "sortParticles", TYPEID< bool >(), Offset(sortParticles, MaskEmitterData),
		"If true, particles are sorted furthest to nearest.");

	addField( "reverseOrder", TYPEID< bool >(), Offset(reverseOrder, MaskEmitterData),
	...
	addField( "textureName", TYPEID< StringTableEntry >(), Offset(textureName, MaskEmitterData),
		"Optional texture to override ParticleData::textureName." );

	addField( "alignParticles", TYPEID< bool >(), Offset(alignParticles, MaskEmitterData),
		"If true, particles always face along the axis defined by alignDirection." );

	addProtectedField( "alignDirection", TYPEID< Point3F>(), Offset(alignDirection, MaskEmitterData), &MaskEmitterData::_setAlignDirection, &defaultProtectedGetFn,
		"The direction aligned particles should face, only valid if alignParticles is true." );

	addField( "highResOnly", TYPEID< bool >(), Offset(highResOnly, MaskEmitterData),
	...
	addField( "renderReflection", TYPEID< bool >(), Offset(renderReflection, MaskEmitterData),
		"Controls whether particles are rendered onto reflective surfaces like water." );

	//@}

	endGroup( "MaskEmitterData" );
	/*
	addField( "Sticky", TYPEID< bool >(), Offset(sticky, MaskEmitterData),
	"If true then bla." );*/

	Parent::initPersistFields();
}

bool MaskEmitterData::_setAlignDirection( void *object, const char *index, const char *data )
{
	MaskEmitterData *p = static_cast<MaskEmitterData*>( object );
...
void MaskEmitterData::packData(BitStream* stream)
{
	...
	if(stream->writeFlag(pixelMaskPath != 0 && !Torque::Path(pixelMaskPath).isEmpty()))
		stream->writeString(pixelMaskPath);
...
void MaskEmitterData::unpackData(BitStream* stream)
{
	...
	pixelMaskPath = (stream->readFlag()) ? stream->readSTString() : 0;
	if(pixelMaskPath != 0)
		pMask.loadFromImage(pixelMaskPath, 1);
...
bool MaskEmitterData::onAdd()
{
	...
	//      Con::errorf(ConsoleLogEntry::General, "MaskEmitterData: Not going to work.  Fix it!");
	//      return false;
	//   }
	...
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) period < 1 ms", getName());
		ejectionPeriodMS = 1;
	}
	if( periodVarianceMS >= ejectionPeriodMS )
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) periodVariance >= period", getName());
		periodVarianceMS = ejectionPeriodMS - 1;
	}
	if( ejectionVelocity < 0.0f )
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) ejectionVelocity < 0.0f", getName());
		ejectionVelocity = 0.0f;
	}
	if( velocityVariance < 0.0f )
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) velocityVariance < 0.0f", getName());
		velocityVariance = 0.0f;
	}
	if( velocityVariance > ejectionVelocity )
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) velocityVariance > ejectionVelocity", getName());
		velocityVariance = ejectionVelocity;
	}
	if( ejectionOffset < 0.0f )
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) ejectionOffset < 0", getName());
		ejectionOffset = 0.0f;
	}

	if ( softnessDistance < 0.0f )
	{
		Con::warnf( ConsoleLogEntry::General, "MaskEmitterData(%s) invalid softnessDistance", getName() );
		softnessDistance = 0.0f;
	}

	if (particleString == NULL && dataBlockIds.size() == 0) 
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) no particleString, invalid datablock", getName());
		return false;
	}
	...
	{
		Con::errorf(ConsoleLogEntry::General, "MaskEmitterData(%s) particle string too long [> 255 chars]", getName());
		return false;
	}

	if( lifetimeMS < 0 )
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) lifetimeMS < 0.0f", getName());
		lifetimeMS = 0;
	}
	if( lifetimeVarianceMS > lifetimeMS )
	{
		Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) lifetimeVarianceMS >= lifetimeMS", getName());
	...
	if( particleString != NULL )
	{
		...
		{
			Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) invalid particles string.  No datablocks found", getName());
			delete [] tokCopy;
			return false;
		}    
		...
		for (U32 i = 0; i < dataBlocks.size(); i++) 
		{
			...
			{
				Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) unable to find particle datablock: %s", getName(), dataBlocks[i]);
			}
			else 
			{
		...
		{
			Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) unable to find any particle datablocks", getName());
			return false;
		}
	}
...
// preload
//-----------------------------------------------------------------------------
bool MaskEmitterData::preload(bool server, String &errorStr)
{
	...
	for (U32 i = 0; i < dataBlockIds.size(); i++) 
	{
		...
		if (Sim::findObject(dataBlockIds[i], pData) == false)
			Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) unable to find particle datablock: %d", getName(), dataBlockIds[i]);
		else
			particleDataBlocks.push_back(pData);
	}
	...
	if (!server)
	{
		...
		else if (particleDataBlocks.size() > 1)
		{
			...
			for (S32 i = 1; i < particleDataBlocks.size(); i++)
			{
				...
				{
					Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) particles reference different textures.", getName());
					break;
				}
			}
		}
	...
	if (blendStyle == ParticleRenderInst::BlendUndefined && particleDataBlocks.size() > 0)
	{
		...
		for (S32 i = 1; i < particleDataBlocks.size(); i++)
		{
			...
			{
				Con::warnf(ConsoleLogEntry::General, "MaskEmitterData(%s) particles have inconsistent useInvAlpha settings.", getName());
				break;
			}
		}
...
//-----------------------------------------------------------------------------
void MaskEmitterData::allocPrimBuffer( S32 overrideSize )
{
...
MaskEmitter::MaskEmitter()
{
	...
	// MaskEmitter should be allocated on the client only.
...
{
	mDataBlock = dynamic_cast<MaskEmitterData*>( dptr );
...
// getCollectiveColor
//-----------------------------------------------------------------------------
ColorF MaskEmitter::getCollectiveColor()
{
	U32 count = 0;
...
// prepRenderImage
//-----------------------------------------------------------------------------
void MaskEmitter::prepRenderImage(SceneRenderState* state)
{
	...
	PROFILE_SCOPE(MaskEmitter_prepRenderImage);
...
// setSizes
//-----------------------------------------------------------------------------
void MaskEmitter::setSizes( F32 *sizeList )
{
...
// setColors
//-----------------------------------------------------------------------------
void MaskEmitter::setColors( ColorF *colorList )
{
	for( int i=0; i<ParticleData::PDC_NUM_KEYS; i++ )
	{
...
// deleteWhenEmpty
//-----------------------------------------------------------------------------
void MaskEmitter::deleteWhenEmpty()
{
	...
	AssertFatal(isProperlyAdded(), "MaskEmitter must be registed before calling deleteWhenEmpty");
	AssertFatal(!mDead, "MaskEmitter already deleted");
	AssertFatal(!isDeleted(), "MaskEmitter already deleted");
	AssertFatal(!isRemoved(), "MaskEmitter already removed");
	...
	if (okToDelete)
	{
		...
		if( !n_parts )
		{
			...
			AssertFatal( getSceneManager() != NULL, "MaskEmitter not on process list and won't get ticked to death" );
	}
}
...
// emitParticles
//-----------------------------------------------------------------------------
void MaskEmitter::emitParticles(const U32      numMilliseconds,
	MaskEmitterNode* node)
{
	if( mDead ) return;
...
bool MaskEmitter::addParticle(MaskEmitterNode* nodeDat)
{
	if(!nodeDat->currentlyShuttingDown() && isProperlyAdded())
	{
		...
		Point2F pxPt = mDataBlock->pMask.getRandomUnitPixel();
		if(pxPt == Point2F::Max)
			return false;
		F32 relx, rely;
		if(nodeDat->standAloneEmitter){
			...
			relx = pxPt.x * mDataBlock->radius;
			rely = pxPt.y * mDataBlock->radius;
		}
		F32 x = relx;
		F32 y = rely;
		F32 z = 0;
		...
		Point3F normal;
		if(nodeDat->grounded)
			GetTerrainHeightAndMat(p.x,p.y,p.z,mat,normal);
		n_parts++;
...
bool MaskEmitter::GetTerrainHeightAndMat(const F32 &x, const F32 &y, F32 &height, StringTableEntry &mat, Point3F &normal)
{
	PROFILE_SCOPE(MaskEmitGetTerHeight);
...
// processTick
//-----------------------------------------------------------------------------
void MaskEmitter::processTick(const Move*)
{
...
// advanceTime
//-----------------------------------------------------------------------------
void MaskEmitter::advanceTime(F32 dt)
{
	...
	AssertFatal( n_parts >= 0, "MaskEmitter: negative part count!" );
...
//-----------------------------------------------------------------------------
void MaskEmitter::updateKeyData( Particle *part )
{
...
// Update particles
//-----------------------------------------------------------------------------
void MaskEmitter::update( U32 ms )
{
	// TODO: Prefetch
	PROFILE_SCOPE(MaskEmitUpdate);
	for (Particle* part = part_list_head.next; part != NULL; part = part->next)
	{
		F32 t = F32(ms) / 1000.0;
...
int QSORT_CALLBACK mask_cmpSortParticles(const void* p1, const void* p2)
{
...
void MaskEmitter::copyToVB( const Point3F &camPos, const ColorF &ambientColor )
{
	static Vector<SortParticle> orderedVector(__FILE__, __LINE__);

	PROFILE_START(MaskEmitter_copyToVB);

	PROFILE_START(MaskEmitter_copyToVB_Sort);
	...
	if (mDataBlock->sortParticles)
	{
		...
		dQsort(orderedVector.address(), orderedVector.size(), sizeof(SortParticle), mask_cmpSortParticles);
	}
	PROFILE_END();
	...
	{
		PROFILE_START(MaskEmitter_copyToVB_Orient);
	...
	{
		PROFILE_START(MaskEmitter_copyToVB_Aligned);
	...
	{
		PROFILE_START(MaskEmitter_copyToVB_NonOriented);
	...
	mVertBuff.unlock();
#else
	PROFILE_START(MaskEmitter_copyToVB_LockCopy);
...
//-----------------------------------------------------------------------------
void MaskEmitter::setupBillboard( Particle *part,
	Point3F *basePts,
...
//-----------------------------------------------------------------------------
void MaskEmitter::setupOriented( Particle *part,
...
void MaskEmitter::setupAligned( const Particle *part, 
	const ColorF &ambientColor,
	ParticleVertexType *lVerts )
{
...
bool MaskEmitterData::reload()
{
	...
	{
		Con::errorf( "MaskEmitterData(%s) has an empty particles string.", getName() );
	...
	for( U32 i = 0; i < numUnits; ++ i )
	{
		...
		{
			Con::errorf( ConsoleLogEntry::General, "MaskEmitterData(%s) unable to find particle datablock: %s", getName(), dbName );
			continue;
		}
	...
	{
		Con::errorf( ConsoleLogEntry::General, "MaskEmitterData(%s) unable to find any particle datablocks", getName() );
		mReloadSignal.trigger();
		return false;
	}
